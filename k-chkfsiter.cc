...
chkfs_fileiter& chkfs_fileiter::find(size_t off) {
    ...
    unsigned bi = min(off, chkfs::maxsize) / blocksize;
...
void chkfs_fileiter::next() {
    while (1) {
        ...
        if (biclass > 1 && !indirect2_entry_) {
            off_ = chkfs::maxsize - 1;
        } else if (biclass > 0 && !indirect_entry_) {
            off_ = biclass * chkfs::nindirect * blocksize
                + chkfs::maxdirectsize - 1;
        }
    }
}
...
int chkfs_fileiter::map(blocknum_t bn) {
    ...
    if (bi >= chkfs::ndirect
        ...
        bufentry* iptr_entry =
            bi >= chkfs::ndirect + chkfs::nindirect
    ...
    bufentry* entry = bi >= chkfs::ndirect ? indirect_entry_ : ino_entry_;
    bc.get_write(entry);
    dptr_ = get_dptr(bi);
    *dptr_ = bn;
    bc.put_write(entry);
...
inline constexpr unsigned chkfs_fileiter::iclass(unsigned bi) {
    return (bi + chkfs::nindirect - chkfs::ndirect)
        / chkfs::nindirect;
}
